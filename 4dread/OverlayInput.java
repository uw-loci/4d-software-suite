...
class OverlayInput
{
  ...
  static String input_directory =
    "/home/curtis/java/4dread/CellLineage2/UnAnnotated/";
    ...
    */
		...
		is = getFileInputStream(new Frame("dummy"), filename);

		readOverlayFromDisk(input_directory, object_array, header_array, focal_plane, frame);
    ...
    */
		...
		if (overlay_index != NOT_FOUND)
		{
			...
			try
			{
				buffer = getByteArrayInputStream(input_directory, filename);
			}
			catch (Exception e)
			{
				return;
	...
	static FileInputStream getFileInputStream (Frame parent, String filename) throws Exception
	{
		...
		fd = new FileDialog(parent, "Open Overlay...", FileDialog.LOAD);
		fd.setFile(filename);
		fd.setVisible(true);
		...
		try
		{
			os = getFileInputStream(input_directory, input_filename);
		}//try
		catch(IOException e)
		{
